...
public class TestGame {
	...
	public void testCreateGrid() {
		...
		int gridSize = game.getGrid().getRows().size();
		...
		for(int i=0; i<gridSize; i++) {
			for(int j=0; j<gridSize; j++) {
				...
				if(game.getGrid().getRows().get(j).getCells().get(i).getDown().equals(CellComponents.APERTURE)) // We thought that every grid creation requires also a gridsize creation. Since there are only one gridsize creation, we used this one for all and called it at every turn instead of creating new grids.
					flag = true;
				else if(game.getGrid().getRows().get(j).getCells().get(i).getUp().equals(CellComponents.APERTURE))
					flag = true;
				else if(game.getGrid().getRows().get(j).getCells().get(i).getLeft().equals(CellComponents.APERTURE))
					flag = true;
				else if(game.getGrid().getRows().get(j).getCells().get(i).getRight().equals(CellComponents.APERTURE))
					flag = true;
		...
		for(int i=0; i<gridSize; i++) {
			for(int j=0; j<gridSize; j++) {
				if(game.getGrid().getRows().get(j).getCells().get(i).getDown().equals(CellComponents.EXIT))
					checkExit = true;
				else if(game.getGrid().getRows().get(j).getCells().get(i).getUp().equals(CellComponents.EXIT))
					checkExit = true;
				else if(game.getGrid().getRows().get(j).getCells().get(i).getLeft().equals(CellComponents.EXIT))
					checkExit = true;
				else if(game.getGrid().getRows().get(j).getCells().get(i).getRight().equals(CellComponents.EXIT))
					checkExit = true;
			}
		}
